...
def _t_label_long_win_tb_eth(md: dict[str, pd.DataFrame]) -> pd.Series:
    """ETH-native long-win triple-barrier label on 1h bars (2026-06-02).

    Identical mechanics to :func:`_t_label_long_win_tb` (k_tp=1.5,
    k_sl=1.0, horizon=24 bars = 24h, atr_window=14) but uses ETHUSDT.
    """
    horizon_bars = 24
    k_tp = 1.5
    k_sl = 1.0
...
def _t_label_long_win_tb_eth_4h(md: dict[str, pd.DataFrame]) -> pd.Series:
    """ETH-native long-win triple-barrier label on 4h bars (2026-06-02).

    Same mechanics (k_tp=1.5, k_sl=1.0, horizon=24 bars = 96h, atr_window=14)
    on ETHUSDT 4h price data. Required by directional_eth_ofi_4h_v1 so 4h
    OFI features align natively to 4h bars.
    """
    horizon_bars = 24
    k_tp = 1.5
    k_sl = 1.0
    ...
    last_t = n - horizon_bars
    if last_t <= 0:
        return pd.Series(out, index=df.index, name="label_long_win_tb_eth_4h_v1")
    h_win = sliding_window_view(h, horizon_bars)[1 : 1 + last_t]
    lo_win = sliding_window_view(lo, horizon_bars)[1 : 1 + last_t]
    entry = c[:last_t]
    atr_v = atr[:last_t]
...
DERIVED_LABELS: dict[str, DerivedFeature] = {
    ...
    # 2026-06-02: ETH-native long-win triple-barrier label (1h bars).
    ...
    # 2026-06-02: ETH-native long-win triple-barrier label (4h bars).
    # Same mechanics (k_tp=1.5, k_sl=1.0, horizon=24, atr_window=14) on
    # ETHUSDT 4h price data — 24-bar horizon = 96h (4 days) holding period.
    # Required by directional_eth_ofi_4h_v1 so the 4h OFI features align
    # natively to 4h bars without the row-count collapse that occurs when
    # the 1h label is joined to 4h features (100 rows → < 1000 floor).
    "label_long_win_tb_eth_4h_v1": DerivedFeature(
        name="label_long_win_tb_eth_4h_v1",
        family="label",
        required_symbols=("ETHUSDT",),
        transformer=_t_label_long_win_tb_eth_4h,
        pit_safe=False,
    ),
}
